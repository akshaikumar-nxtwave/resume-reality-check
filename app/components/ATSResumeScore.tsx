...
const TABS = [
  ...
  { id: "jd-match", label: "JD Match", icon: Briefcase },
  ...
  const TabBar = (
    <div className="sticky top-0 z-20 border-b border-border bg-white/50 backdrop-blur">
      <div className="mx-auto flex max-w-6xl gap-1 overflow-x-auto px-6 py-3">
        {TABS.filter(({ id }) => id !== "jd-match" || d.jd_evaluation).map(({ id, label, icon: Icon }) => {
          const active = tab === id;
          return (
            <button
              key={id}
              onClick={() => setTab(id)}
              className={`inline-flex items-center gap-2 whitespace-nowrap rounded-full border px-4 py-2 text-sm font-medium transition-all ${active
  ...
  return (
    <div className="min-h-screen bg-white/50">
      ...
      <main className="mx-auto max-w-6xl px-6 py-10">
                    ...
                    {Object.entries(d.categories || {}).map(([key, c]) => {
                      ...
                      return (
                        <div key={key} className="space-y-2">
                          ...
                          <ProgressBar value={c.score} max={c.weight} tone={scoreTone((c.score / c.weight) * 100)} />
        ...
        {/* JD MATCH */}
        {tab === "jd-match" && d.jd_evaluation && (
          <div className="space-y-6">
            {/* Top Match Score & Overview Row */}
            <div className="grid gap-6 md:grid-cols-3">
              <div className="md:col-span-1 flex flex-col justify-center items-center rounded-3xl border border-border bg-card p-6 shadow-elegant text-center">
                <div className="space-y-3">
                  ...
                  <div className="inline-flex items-center justify-center relative w-32 h-32">
                    <svg className="w-full h-full transform -rotate-90">
                      ...
                      <circle
                        ...
                        cy="64"
                        r="52"
                        className={`${(d.jd_evaluation.score ?? 0) >= 80
                          ? "stroke-success"
                          : (d.jd_evaluation.score ?? 0) >= 60
                            ? "stroke-indigo-600"
                            : (d.jd_evaluation.score ?? 0) >= 40
                              ? "stroke-warning"
                              : "stroke-destructive"
                          }`}
                        strokeWidth="10"
                        strokeDasharray={2 * Math.PI * 52}
                        strokeDashoffset={2 * Math.PI * 52 * (1 - (d.jd_evaluation.score ?? 0) / 100)}
                        strokeLinecap="round"
                        fill="transparent"
                      />
                    </svg>
                    <span className="absolute font-serif text-3xl font-black text-foreground">
                      {d.jd_evaluation.score ?? "—"}
                    </span>
                  </div>
                  <div className="pt-1">
                    <Tag tone={scoreTone((d.jd_evaluation.score ?? 0) * 1)}>
                      {(d.jd_evaluation.score ?? 0) >= 80
                        ? "Strong Match"
                        : (d.jd_evaluation.score ?? 0) >= 60
                          ? "Good Match"
                          : (d.jd_evaluation.score ?? 0) >= 40
              ...
              <div className="md:col-span-2 flex flex-col justify-between rounded-3xl border border-border bg-card p-6 shadow-elegant">
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-sm text-foreground leading-relaxed">
                      {fallback(d.jd_evaluation.summary, "No summary available")}
                    </p>
                  </div>
                  <div>
                    <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1">
                    ...
                    <p className="text-sm text-foreground leading-relaxed">
                      {fallback(d.jd_evaluation.experience_match, "No experience match details detected")}
                    </p>
                  </div>
                </div>
              </div>
            ...
            {/* Matching details row */}
            <div className="grid gap-6 md:grid-cols-2">
              <SectionCard
                title="Matching Skills"
                icon={<Code2 className="h-5 w-5" />}
              >
                {isMissing(d.jd_evaluation.matching_skills) ? (
                  <Empty>No matching skills found in resume</Empty>
                ) : (
                  <div className="flex flex-wrap gap-2">
                    {(d.jd_evaluation.matching_skills || []).map((s) => (
                      <Tag key={s} tone="success">
                        {s}
                      </Tag>
                    ))}
              ...
              >
                {isMissing(d.jd_evaluation.matching_projects) ? (
                  <Empty>No projects matching the JD requirements found</Empty>
                ) : (
                  <ul className="space-y-2">
                    {(d.jd_evaluation.matching_projects || []).map((p) => (
                      <li key={p} className="flex gap-2 text-sm font-medium text-slate-800 bg-slate-50 border border-slate-100 p-2.5 rounded-xl">
            ...
            {/* Gap Analysis & Project Recommendations Section */}
            <SectionCard
              title="Resume Gap Analysis & Actionable Suggestions"
              icon={<AlertTriangle className="h-5 w-5 text-warning" />}
              description="Review missing JD skills and how to address them by building new projects or extending existing ones"
            >
              {isMissing(d.jd_evaluation.gap_analysis) ? (
                <Empty>No skill gaps identified. Your resume matches the job description perfectly!</Empty>
              ) : (
                <div className="space-y-6">
                  {(d.jd_evaluation.gap_analysis || []).map((gap, i) => {
                    const hasFoundation = gap.foundation_status === 'adjacent';
                    return (
                      <div key={i} className={`p-5 rounded-2xl border transition-all duration-200 ${hasFoundation
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-muted-foreground">• Missing Skill</span>
                          </div>

                          <Tag tone={hasFoundation ? "accent" : "warning"}>
                            {hasFoundation ? "Adjacent Foundation Found" : "No Related Foundation"}
                          </Tag>
                        </div>

                        {/* Related Skills / Projects callout if present */}
                        {gap.related_skills_or_projects && gap.related_skills_or_projects.length > 0 && (
                          <div className="mb-3 text-xs text-muted-foreground flex items-center gap-1.5 flex-wrap">
                            <span className="font-semibold text-slate-600">Detected in resume:</span>
                            {gap.related_skills_or_projects.map((item, idx) => (
                              <span key={idx} className="bg-slate-100 border border-slate-200/60 text-slate-700 px-2 py-0.5 rounded">
                                {item}
                              </span>
                            ))}
                          </div>
                        ...
                        <div className="mt-2 text-sm leading-relaxed text-slate-700">
                          <div className="flex gap-2">
                            <div className="font-semibold text-indigo-600 shrink-0">
                              {hasFoundation ? "Project Extension:" : "New Project Idea:"}
                            </div>
                            <div>
                              {gap.actionable_suggestion}
                            </div>
                          </div>
                        </div>
                      </div>
            ...
            {!isMissing(d.jd_evaluation.notes) && (
              <SectionCard
                title="Evaluation Notes"
                icon={<FileText className="h-5 w-5" />}
              >
                <p className="text-sm leading-relaxed text-muted-foreground">
                  {d.jd_evaluation.notes}
                </p>
              </SectionCard>
            )}
          </div>
                  ...
                  <div className="space-y-4">
                    {Object.entries(d.skills?.skill_score || {}).map(([k, v]) => (
                      <div key={k}>
                        ...
                        <ProgressBar value={Number(v)} max={10} tone={scoreTone(Number(v) * 10)} />
                      </div>
                    ))}
                  </div>
                )}
                ...
                <div>
                  ...
                  {isMissing(d.layout_insights?.section_order) ? (
                    ...
                    <ol className="space-y-2">
                      {d.layout_insights!.section_order && d.layout_insights!.section_order.map((s, i) => (
                        <li
                          key={s}
              ...
              <div className="mt-6 grid gap-5 md:grid-cols-3">
                <div>
                  ...
                  {isMissing(d.layout_insights?.section_order_issues) ? (
                    ...
                    <ul className="space-y-1.5">
                      {d.layout_insights!.section_order_issues && d.layout_insights!.section_order_issues.map((x) => (
                ...
                <div>
                  ...
                  {isMissing(
                    ...
                    <div className="flex flex-wrap gap-2">
                      {d.layout_insights!.missing_recommended_sections && d.layout_insights!.missing_recommended_sections.map(
                        (x) => (
                ...
                <div>
                  ...
                  {isMissing(d.layout_insights?.unnecessary_sections) ? (
                    ...
                    <div className="flex flex-wrap gap-2">
                      {d.layout_insights!.unnecessary_sections && d.layout_insights!.unnecessary_sections.map((x) => (
